...
Given('{tvar} ← canvas\({int}, {int})') do |var, int, int2|
  instance_variable_set(var, RT::Canvas[int, int2])
end

Then("{tvar}.{canvas_attr} = {int}") do |canvas, attr, value|
...
When("every pixel of {tvar} is set to {color}") do |canvas, color|
  instance_variable_get(canvas).pixels=(color)  
end

Then("{tvar} ends with a newline character") do |ppm|
